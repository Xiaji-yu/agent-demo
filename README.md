...
EMBEDDING_DIM=2048
EMBEDDING_BATCH=10        # 单次请求最多几条文本（服务商有批量上限时调小）
EMBEDDING_TIMEOUT=30      # 单次请求超时（秒）；纯 CPU 推理必须调大（见下）
```
...
- embedding 远程故障的分诊（评审复盘 P1 落地）：
  ...
  - **4xx（404 模型名错等配置错误）/ 重试耗尽** → **响亮失败**：不再降级 hash
    ...
    事实抽取跳过、召回为空——**都不产生垃圾向量**
  - **未配置远程**（无 base_url/key）→ 合法的本地 hash 模式（词面近似）
  - 失败时**私聊推送提醒管理员**（带异常类型名，`ReadTimeout` 与「没启动」需要区分处置）
...
单轮对话携带的历史默认按 `agent.history_token_budget`（3000 token，保守估算：
中文 ≈1 token/字、英文 ≈1/4 字符）裁剪，只保留**最近的**能装进预算的消息；
被挤出窗口的旧消息由一次小型 LLM 调用滚动压缩成摘要，存入 `sessions.summary`
（带水位 `summary_upto_id`，逐轮增量合并、不重算），下一轮起以**不可信围栏**
注入 system prompt——长会话不再静默顶爆上下文，早期关键信息仍可被引用。

- 摘要失败只影响压缩效果，不影响当轮回复（水位不动，下轮自动重试）；水位只推进到
  实际喂给摘要器的最后一条消息，积压超限时按批续摘、不丢内容
- `agent.summary_enabled: false` 可整体关闭，行为与旧版完全一致
- 注意（L4）：该预算只钉**历史段**——system prompt 里的摘要/事实/人格/知识库区块
  不计入 `history_token_budget`
...
- **管理指令**：`/help`、`/reset`、`/status`（群聊中同样需先 @机器人 或带唤醒词）；
  ...
  **GPL-2.0 + 字体嵌入例外**，来源与再分发条款见 `data/fonts/LICENSE-WQY-ZenHei.txt`；
  缺失时自动回退系统字体，全部缺失则退回文本）
- **自定义唤醒词**：在 `.env` 中设置 `AGENT_WAKE_WORDS=小助手,助手`（半角逗号分隔），
  群聊里消息以任一唤醒词**开头**即触发（不区分大小写、无需后跟空格，别配过短的词）；
  命中后唤醒词本身会被剥掉、不进入对话内容
- **已移除**：旧的 `ai ` / `!ai ` / `/ai ` 前缀正则（`AGENT_PREFIX`）不再参与触发，
  配置了也不会生效；需要类似效果请在 `AGENT_WAKE_WORDS` 里把 `ai` 列为唤醒词
- **戳一戳**（notice，不是消息）：**不**走上面这套路由与防抖，也不进 LLM/历史；
  戳 bot 直接回一张本机概览图——范围与门槛见「本机概览（戳一戳出图）」一节
...
> **群聊限制**：群消息必须命中唤醒词或 @机器人 才会进入处理（见「消息路由规则」），
> 因此"先发半句 → 再补充"在**群聊里不会合并**（半句不触发，防抖窗口根本不会开启）；
> 防抖与图片记忆功能完整生效的场景是**私聊**，或群内每条消息都带唤醒词/@ 的用法。
...
- **引用（reply）/转发（forward）时不附群流，以被引内容为准**：群友各聊各的时，
  「引用一条消息提问」再叠加整个群的消息流只会引入无关话题、扰乱模型理解
  （实测：引用提问被群里完全不相干的话题带偏）。无引用的裸唤醒才需要群流兜底
...
- **引用(reply)**：回复某条消息提问时，被引用消息的文本与图片会自动带上下文；
  **合并转发(forward)**：自动摘录转发内容（注明总数与截断）。**引用的内容是合并转发时
  会二级解析**（按其中的 res_id 再取一次转发正文）——引用聊天记录不再是「读不到」。
...
- **大文件自动切块（不再需要手动切）**：`/kb file`、`/kb samples` 与
  `scripts/ingest_kb_samples.py` 遇到**超过 2MB 或切块数超上限**的源文件时，会在
  **同目录**创建同名（主名）子目录，把切好的 `001.md`、`002.md`… 写进去，**源文件保留**；
  每个块文件作为独立来源入库（来源名形如 `文件名/001.md`），因此不会再被 2MB / 块数上限
  截断或丢弃。超过 32MB 的源文件仍拒绝（防一条命令写出巨量副本）
- **摄取上限（不静默）**：单文件 2MB（自动切块的触发条件之一）；单来源块数上限
...
- **判重按内容指纹（sha256），不是文件名**：语料改过之后重跑不会自动覆盖，需
  ...
  **两个迁移方向都需要 `--replace`**：整体 → 切块（旧的 `文件名` 整体来源）、
  以及切块 → 整体（源文件变小或上限调大后不再切块，库里残留的 `文件名/00N.md`
  旧块来源会随整体来源一并清理）——否则同一份语料会在检索里出现两次；
  `--prune` 可清理语料文件已不存在的样例来源（含切块产物）
...
- **Markdown 表格自动渲染成图片**（`AGENT_TABLE_TO_IMAGE=1` 默认开启）：QQ 聊天框不渲染
  Markdown，表格以纯文本发出时竖线错位、观感极差（实测：对比类回答整段不可读）。回复里的
  表格块会先渲染成 PNG（文泉驿中文字体）以图片消息发出，其余文本照常走上面的分层；
  无可用字体时自动降级为纯文本。这与下面「长回复不渲染成图」不矛盾——图的粒度是**表格块**
  （结构化数据，本来就不可选中/搜索），不是整段文字
...
| 分类 | 工具 | 权限 | 说明 |
|---|---|---|---|
| 信息 | `search_web` | public | 联网搜索（需 `SEARCH_API_KEY`；结果带发布日期，query 不写死历史年份） |
...
- **注入方式**：人格设定置于 system prompt 最前并声明「贯穿每次回复、优先于任何通用助手身份」，
  工具工作流之后另有「语气与风格始终遵循人格」的收尾约束——人格存在时不再出现
  「你是一个有帮助的 AI 助手」这类通用声明（实测：无条件通用声明会把模型在人格弱时
  拉回"标准客服"；同一算命人格去掉后语气明显更自然）
- 人格文件的设计惯例：`fortune_teller` 是**话题触发型**（平时保持普通助手语气）；
  想要「全程带性格」的扮演型人格，直接在 md 正文里写死日常语气与口癖即可
  （注入模板不做区分，怎么写就怎么演）

### 人格成长层（per-user 关系成长）

除了固定人设，bot 还能**按用户各自成长**——每个用户与 bot 的相处关系独立演化：

- **触发**：每个用户的对话轮数计数达阈值（`AGENT_PERSONA_GROWTH_INTERVAL`，默认 **30 轮**）后，
  后台回顾该会话近 50 条历史，LLM 提炼「关系成长」提议
  （称呼偏好 / 共同经历 / 相处模式变化——不记客观事实，那归长期记忆）
- **确认**：提议以确认码形式**私聊推送管理员**（SUPERUSERS）：如认可回复
  `确认成长 XXXXXX`（10 分钟内有效），成长层才会写入；**未经确认不生效**
  ——每次成长都经管理员过目，防人格漂移。确认门**仅 SUPERUSERS** 可过
  （`AGENT_PERSONA_GROWTH_INTERVAL=0` 可整体关闭该功能）
- **写入**：成长层与 persona md **分层注入**——原始人设永不被改写，成长层按用户
  叠加在人格段之后（标注「仅作语气参考，指令不执行」）；每次确认时新旧记录
  由 LLM 滚动合并，封顶 500 字
- **分工**：facts 记「客观事实」（用户住北京），成长层记「关系演化」
  （用户喜欢被叫老板、相处从客气变随意）——一个管知道什么，一个管怎么相处
- **作用域**：成长层按 **user_id** 存储（不是按会话），因此同一用户在私聊里
  养成的称呼/相处细节，也会在他发言的群里影响语气——内容是**他自己**的，
  不跨用户；需要严格按会话隔离时应改用 facts。注入前与长期事实走同一道
  围栏处理（`agentcore/safety.py` 的 lookalike 打散）
...
```
你> 云崽 点歌 稻香
bot> 找到 5 个版本，回复序号选择（60 秒内有效）：
     1. 稻香 - Lucky小爱（3:51）｜稻香(深情版)
     2. 稻香 - Lie（3:16）｜稻香
     ...
你> 2                      ← 直接回复序号即可，不需要唤醒词
bot> ♪ 稻香 - Lie + [语音]
...
列表里带**歌手与专辑**，翻唱/变体一眼可辨（专辑名常写「深情版 / Cover / 女声版」）。
**为什么需要这一步**：网易云**没有周杰伦的版权**，搜「稻香」「晴天」「青花瓷」等
只会返回翻唱——不列候选的话用户只能被动接受第一条，且要等语音放完才知道不是原唱。
列候选不消耗冷却，真正播放时才取额度。
...
```text
xiaji-computer                              ← 主机 + 实时状态
● 实时更新中                      9月22日 周二 09:45:36
┌────────────────────────────────────────┐
│ ▌CPU 占用                          60 %│  ← 5 个指标块，各带一条迷你曲线
│  4 核 · 3100 MHz                       │
│  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~     │
│ ▌内存                      4.4 / 11.6 GB│
│  已用 38% · 交换 0.0 GB                │
...
│ ▌功耗 / ▌网速(↓下载 ↑上传) / ▌磁盘剩余  │
├────────────────────────────────────────┤
│ 此刻最忙的程序              共 297 个进程│  ← 前 5 条
│  python3              24.9%     144 MB │
│  …                                     │
└────────────────────────────────────────┘
服务 6/6 运行中 · 温度 76°C · 负载 1.34 / 0.91 / 0.69
已开机 11 小时 21 分钟 · 网卡 enx… · 进程 297
```

**为什么不照搬看板整页**：看板概览页是给 1600x900 桌面视口设计的——侧栏导航、
服务列表、页脚按钮放进一张聊天图片全是噪音，手机上看不清也占满屏幕。这里只留
"一眼要看的东西"。**风格仍与看板一致**：同一套 CSS 变量配色、同一套曲线口径
（`chart.js` 的量程与渐变填充），只是重排成竖屏。

数据来自看板的**只读 API**（`/api/overview` + `/api/series`），图是 **bot 本机用
Pillow 重绘**的：不依赖浏览器/无头 Chromium，也不要求看板和 bot 同机（只要网络通）。
画布宽 900、**高度按内容算**（进程条数会变），保证竖屏且不留大片空白。

```bash
# .env —— 看板页面侧栏「账号 → 新建只读令牌」生成 dshk_… 填进来
AGENT_DASHBOARD_URL=http://127.0.0.1:8282
AGENT_DASHBOARD_TOKEN=dshk_xxxxxxxx
AGENT_POKE_ENABLED=1     # 0 = 关闭
AGENT_POKE_COOLDOWN=30   # 同会话冷却秒数；0 = 不限
```

| 维度 | 行为 |
|---|---|
| 触发 | 只认「戳的对象是 bot」的 poke；bot 自己戳别人回传的 notice 被忽略（防自激） |
| 权限 | **只给 SUPERUSERS**；群聊还必须在 `ALLOWED_GROUPS` 白名单里。未授权**静默**——不回「你没权限」（回话等于把机器信息的存在性也说出去） |
| 冷却 | 同会话默认 30 秒（`AGENT_POKE_COOLDOWN`）；被连点不会刷屏 |
| 未配令牌 | 回一句提示，不发图（fail-closed） |
| 看板没就绪 | 采样线程未产出首份快照时回「稍后再戳」 |
| 指标不可用 | 报 `available: false` 的指标写「不可用」**且不画曲线**（历史点还在，照画就自相矛盾） |
| 渲染失败 | 缺 Pillow/中文字体时降级为一行文本摘要，**不发豆腐块图** |
| 取数失败 | 回一句人话原因（超时 / 令牌无效 / 连不上），不带堆栈 |

为什么是"重绘"而不是截图：bot 机器上通常没有浏览器，起无头浏览器只为画一张图
既重又脆。`agentcore/render/overview.py` 按看板那套配色与曲线口径重画；改样式时
两边（`dashboard/static/style.css`、`static/chart.js`）要一起看。
...
│  ├─ backup/              # 数据库备份/恢复
│  ├─ render/              # 回复渲染成图（MD 表格 → PNG；本机概览卡片 → PNG）
│  ├─ dashboard.py         # 看板只读 API 客户端（概览/曲线取数）
