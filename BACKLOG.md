...
**M 级已修 13 条**（注入面 3、存储契约 2、备份/归档/蒸馏 5、单位换算 2、大小写归一 1；见 FIX 文档第二批）。**M 级剩余 4 条待修**：注入面 3并发/资源 6（备份轮转 2.16s 阻塞、JSONL 4.27s + 整表进内存、图片缓冲最坏 ~350MB、合并可倒序、parts 无上限、无全局并发闸门）+ 存储作用域 2（`resolve_session` 的 `group_id="private"` 同键、`messages_after` 未注册会话判定）—— 见报告 §2。
