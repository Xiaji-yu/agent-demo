...
注入边界（见 ``pipeline._build``）：**无引用的裸唤醒才附群流**；本条消息带
reply 或 forward 段时不附——被引/转发内容本身就是最强语境信号，群友各聊各的
时再叠加整个群的消息流只会引入无关话题（实测：引用提问被群里不相干的话题
带偏）。记录照常进行，留待「他们刚才聊啥」这类无自带语境的提问使用。
