...
7. **测试要能抓 bug**：新增/修改用例后，把实现**故意改坏**跑一遍，确认用例**真的失败**再还原
   ...
   回归用例**放进被测模块对应的主题测试文件**（文件头注明来源评审），不再按评审轮新开 `test_review_*` 批次文件；
   双实现行为断言只加在 `tests/test_store_contract.py`，`test_pg_store.py` 只收 PG 独有面（DDL/索引/池/损坏数据）。
...
| Web 总览 | `/agent-web` **只读**（无写接口）；未配 `AGENT_WEB_TOKEN` 则整个面不挂载（fail-closed）；token 只认 `Authorization: Bearer`（**不接受 URL query**）且用 `secrets.compare_digest`；可选 `AGENT_WEB_ALLOW_CIDRS` 源 IP 白名单；响应体**不得**含 api_key/base_url/消息正文。加写入功能前必须同时补审计（`agentcore/diagnostics.py`）+ 二次确认 + 改前备份 |
