...
## 追加功能：多版本候选选择（2026-09-19，真机联调后按用户选择实现）

### 背景（真机发现的产品缺陷，非代码 bug）

用户实测 `点歌 稻香` 拿到的是**翻唱**（`稻香 - Lucky小爱`），于是又问「这个 api 是不是
有点问题」。排查结论：**API 没问题，是网易云没有周杰伦的版权**（其版权在 QQ 音乐独家）：

| 查询 | 搜索首条 | 前 3 条含原唱 |
|---|---|---|
| 晴天 / 青花瓷 / 告白气球 / 七里香 | 全是翻唱（深情版/Cover/女声版） | ❌ |
| 海阔天空 / 后来 / 月亮代表我的心 | Beyond / 刘若英 / 邓丽君 | ✅ |

翻遍前 150 条结果，唯一的"周杰伦"是个**假艺人名**（`周杰伦.`，多个点）；其歌曲直连
取地址返回 `code=404 无 URL`。**换音源才能拿到周杰伦原唱。**

但代码有一处真实缺陷：**盲取 `songs[0]`** ——用户无从选择，且要等语音放完才从
「♪ 稻香 - Lucky小爱」看出不是原唱。

### 实现（用户选定方案：多条时列 Top5 + 回复序号）

- **`_play` 拆成两段**：`_play`（校验 → 搜索 → 时长过滤 → 分流）与 `_play_song`
  （冷却 → 取址 → 下载 → 编码 → 发送）。两条入口（按名点歌 / 序号选择）共用
  `_play_song`，保证冷却、缓存、安全校验、发送语义完全一致。
- **候选列表**：`usable` 截到 5 条；`>1` 条时记录待选并回复
  `序号. 歌名 - 歌手（时长）｜专辑`——**专辑名常写「深情版 / Cover / 女声版」，
  是识别翻唱的关键线索**。
- **回复序号即可选**：新增 `selection_matcher`（priority 4，先于点歌 5 与聊天 10），
  `_selection_rule` 的门是**"该用户在当前会话确有待选项 + 文本是范围内序号"**，
  **不要求唤醒词**（bot 刚问过"回复序号选择"，再要「云崽 2」是多余的）；
  仍保留群白名单 / 私聊 ACL / 自身消息过滤。无待选项时返回 False，一个裸「2」
  照常落回普通聊天。
- **不消耗冷却**：列候选只是询问，**不取**账号级额度；真正播放（`_play_song`）才取。
  （实现时踩过一次：`_play` 里旧的 `try_acquire` 没删干净 → 列完候选立刻播放会被
  自己的冷却拒掉，已修。）
- **待选状态**：`PendingSelections`，按 `(会话, 用户)` 隔离、TTL 60s、取用即消费、
  自动 prune；纯内存、无持久化、不引入队列。
- **序号只认 ASCII 数字**：全角「１」不认（§5 的坑），宁可落回普通聊天也不做
  "看着选了其实没选"。

### 真机链路验证（真搜索/真下载/真编码，仅群发送用桩）

```
① 点歌 稻香 →
   找到 5 个版本，回复序号选择（60 秒内有效）：
   1. 稻香 - Lucky小爱（3:51）｜稻香(深情版)
   2. 稻香 - Lie（3:16）｜稻香
   3. 《稻香》童声 - 卡罗尔（3:31）｜《悬溺》+《空心》+《没有泪点》
   4. 稻香(治愈版) - 周杰伦.、街道办GDC/欧阳耀莹.（2:00）｜终究意难平
   5. 稻香 - 纸飞机（3:23）｜稻香
② 回复 2 → 下载 + silk 584929 字节 → ♪ 稻香 - Lie
③ 无待选项时裸「2」→ 规则不命中（落回普通聊天）
```

### 回归与变异

- `TestParseSelection` / `TestPendingSelections` / `TestCandidateFlow` /
  `TestSelectionWithoutWakeWord` / `TestSelectionHandler` 共 21 条
- 变异复核 6 处（不再列列表、去掉待选校验、无待选也命中、去掉越界检查、
  取用不消费、列表不显示专辑）**全部被捕获**
- **附带修掉一处用例间污染**：`TestMusicEnvReadyGate::test_route_not_imported_when_env_missing`
  故意把 `music_route` 移出 `sys.modules` 却没恢复，导致后续用例 `importlib.reload`
  抛 `ImportError`；已改为 finally 恢复现场，并让 fixture 自我兜底

全量：**1380 passed / 44 skipped**（有 ffmpeg）、**1374 / 50**（无 ffmpeg）；ruff 全绿。
